...
class AdminServices {
  void sellProduct({
    ...
    try {
      final cloudinary = CloudinaryPublic('apiero', 'gttiyu6z');
  ...
  void adsProduct({
    ...
    try {
      ...
      http.Response res = await http.post(
        Uri.parse('$uri/admin/add-ads'),
        headers: {
          'Content-Type': 'application/json; charset=UTF-8',
          'x-auth-token': userProvider.user.token,
        },
        body: advertise.toJson(),
      );
      ...
      httpErrorHandling(
        ...
        context: context,
        onSuccess: () {
          showSnackBar(context, 'Ads Added Successfully!');
          Navigator.pop(context);
        },
      );
    } catch (e) {
  ...
  //get all the products
  Future<List<Advertise>> fetchAllAds(BuildContext context) async {
    final userProvider = Provider.of<UserProvider>(context, listen: false);
    List<Advertise> advertiseList = [];
    try {
      http.Response res =
          await http.get(Uri.parse("$uri/admin/get-ads"), headers: {
        'Content-Type': 'application/json; charset=UTF-8',
        'x-auth-token': userProvider.user.token,
      });
  ...
  void deleteAds({
    ...
    try {
      http.Response res = await http.post(
        Uri.parse('$uri/admin/delete-ads'),
        headers: {
          'Content-Type': 'application/json; charset=UTF-8',
          'x-auth-token': userProvider.user.token,
        },
        body: jsonEncode({'id': advertise.id}),
      );
  ...
  Future<List<Order>> fetchAllOrdersTo(BuildContext context) async {
    final userProvider = Provider.of<UserProvider>(context, listen: false);
    List<Order> orderList = [];
    try {
      http.Response res =
    ...
    try {
      ...
      httpErrorHandling(
        ...
        onSuccess: () {
          ...
          sales = [
            Sales('Maggi', response['maggiEarnings']),
            Sales('Pasta', response['pastaEarnings']),
            Sales('Sandwich', response['sandwichEarnings']),
            Sales('Beverages', response['beveragesEarnings']),
            Sales('Meal', response['mealEarnings']),
          ];
        },
      );
    } catch (e) {
